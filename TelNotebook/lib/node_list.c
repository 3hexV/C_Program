...
DATA randNodeData(void)
{
	...
	strcpy(tmpData.name, randStr(3, 12));	
	strcpy(tmpData.city, randStr(3, 1));	
	strcpy(tmpData.province, randStr(2, 1));	
	strcpy(tmpData.state, randStr(2, 1));
	strcpy(tmpData.tel, randStr(11, 3));
...
/*
	func	: 插入节点 在指定节点后 (char[]类型数据) 
	para	: Node* headNode,char *dataStrId, DATA tmpData  头节点，char[]的数据ID，DATA结构体 
	return	: void
	more	:是否依赖DATA结构体内部数据类型 ：√ 
*/
void insertNodeByPosDataStr(NODE *headNode, char *dataStrId, DATA tmpData){
	...
	while(!strcmp(forNode->data.name, dataStrId)){
		forNode = forNode->next;
	}
...
/*
	func	: 删除节点 / 通过指定数据(char[]类型数据)  ，只删除第一次查询到的  
	para	: Node* headNode,char *dataStrIdd  头节点，(char[]的数据ID 
	return	: int -1:表空 0：成功 
	more	:是否依赖DATA结构体内部数据类型 ：√ 
*/
int delNodeByPosDataStr(NODE *headNode, char *dataStrId){
	...
	while(forNode != NULL){
		if(!strcmp(forNode->data.name, dataStrId)) break;
		forNode = forNode->next;
		tmpNode = tmpNode->next;
	}
...
//		printf("%s|%s\n", dataId, forNode->data.studentInfo.id);
		if(!strcmp(forNode->data.name, dataStrId))
		{
			tmpNode->next = forNode->next;
			free(forNode);
			count++;
...
/*
	func	: 查询打印节点 / 通过指定的数据(char[]类型数据)   
	para	: Node* headNode,int dataId  头节点，char[]的数据ID 
	return	: int 1:成功  0：失败 
	more	:是否依赖DATA结构体内部数据类型 ：√ 
*/
...
NODE *returnFrontEndNode(NODE *headNode){
	...
	if(forNode->next == NULL) return NULL;
...
int listSortFunc(NODE *headNode){
	...
	while(baseP!=endP){
		...
		while(1){
			...
			if(strcmp(standP->data.name, moveP->data.name)>0){
				tmpData = standP->data;
